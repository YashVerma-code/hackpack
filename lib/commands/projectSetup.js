...
import { setupAuthjs, setupClerk } from '../authentication/index.js';
...
export async function runSetupFromState(state) {
  ...
  try {
    ...
    if (!state.fromWizard && state.uiLibrary && ['next', 'svelte', 'vite-react'].includes(framework)) {
    ...
    console.log(chalk.blue(`Creating authentication: ${authentication} ...`));
    if (authentication) {
