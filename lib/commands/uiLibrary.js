...
// UI setup functions (Vue)
import { setupDaisyUi } from '../createVueProject/ui/daisyUi.js';
import { setupVuetify } from '../createVueProject/ui/vuetify.js';
import { setupShadcnVue } from '../createVueProject/ui/shadcnVue.js';
import { setupPrimeVue } from '../createVueProject/ui/primeVue.js';

// Nuxt UI helpers
import { setupShadcnUI as setupShadcnNuxt } from '../createNuxtProject/ui/shadcn.js';
import { setupDaisyUI as setupDaisyUINuxt } from '../createNuxtProject/ui/daisyui.js';

// Astro UI helpers
import { setupShadcnUI as setupShadcnAstro } from '../createAstroProject/ui/shadcn.js';
import { setupDaisyUi as setupDaisyUIAstro } from '../createAstroProject/ui/daisyui.js';

// Angular UI helpers
import { setupAngularMaterial } from '../createAngularProject/ui/angularmaterialui.js';
...
export async function applyUILibrary({ framework, projectName, language, library, state }) {
  ...
  const useTailwind = state && state.styling === 'tailwind';
  ...
  try {
    switch (framework) {
      ...
      case 'vue':
        switch (library) {
          ...
          case 'vuetify':
            ...
            break;
          case 'shadcn-vue':
            await setupShadcnVue(projectName);
            break;
          case 'primevue':
            await setupPrimeVue(projectName, useTailwind);
            break;
          case 'tailwind-only':
            await addTailwind({ framework: 'vue', projectName });
            break;
          case 'plain':
            console.log(chalk.yellow('Plain CSS selected for Vue — no UI packages will be installed.'));
            break;
          default:
            console.log(chalk.red('Unknown UI library for Vue.'));
        }
        break;
      ...
      case 'nuxt':
        switch (library) {
          case 'shadcn':
            await setupShadcnNuxt(projectName, useTailwind);
            break;
          case 'daisyui':
            await setupDaisyUINuxt(projectName);
            break;
          case 'tailwind-only':
            await addTailwind({ framework: 'nuxt', projectName });
            break;
          case 'plain':
            console.log(chalk.yellow('Plain CSS selected for Nuxt — no UI packages will be installed.'));
            break;
          default:
            console.log(chalk.red('Unknown UI library for Nuxt.'));
        }
        break;
      ...
      case 'astro':
        switch (library) {
          case 'shadcn':
            await setupShadcnAstro(projectName, language === 'ts', useTailwind);
            break;
          case 'daisyui':
            await setupDaisyUIAstro(projectName, language === 'ts', useTailwind);
            break;
          case 'tailwind-only':
            await addTailwind({ framework: 'astro', projectName });
            break;
          case 'plain':
            console.log(chalk.yellow('Plain CSS selected for Astro — no UI packages will be installed.'));
            break;
          default:
            console.log(chalk.red('Unknown UI library for Astro.'));
        }
        break;
      ...
      case 'angular':
        switch (library) {
          ...
          case 'material':
            ...
            break;
          case 'tailwind-only':
            await addTailwind({ framework: 'angular', projectName });
            break;
          case 'plain':
            console.log(chalk.yellow('Plain CSS selected for Angular — no UI packages will be installed.'));
            break;
          default:
            console.log(chalk.red('Unknown UI library for Angular.'));
        }
        break;
