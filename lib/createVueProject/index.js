...
import { setupShadcnVue } from "./ui/shadcnVue.js";
...
async function createVueProject({ projectName: initialProjectName, language, styling, uiLibrary }) {
  ...
  if (!useTailwind && ["daisyui","shadcn-vue"].includes(uiLibraryChoice)) {
    console.log(
  ...
  try {
    ...
    if (languageChoice === "ts") {
      createVueAppFlags = [];
      createVueAppFlags.push("--");
      createVueAppFlags.push("--ts");
    ...
    if (useTailwind) {
      ...
      // Prefer vite.config.ts if it exists, fallback to vite.config.js
      ...
      // If no config file found, create vite.config.js fallback
      if (!configPath) {
      ...
      // Inject tailwindcss() into the plugins array if we have a config
      if (viteConfig) {
      ...
      if (fs.existsSync(cssPath)) {
        ...
        if (!cssContent.includes('@import "tailwindcss')) {
          ...
          console.log("Tailwind imports already present in main.css.");
        }
      } else {
    ...
    if (uiLibraryChoice !== "none" && uiLibraryChoice !== "tailwind-only") {
      switch (uiLibraryChoice) {
        ...
        case "vuetify":
          ...
          break;
        case "shadcn-vue":
          await setupShadcnVue(projectName, languageChoice);
          break;
        case "primevue":
