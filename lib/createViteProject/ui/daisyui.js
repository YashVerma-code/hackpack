...
async function ensureTailwindIsInstalled() {
  try {
    ...
    await installTailwindCSS(languageChoice);
  }
}

async function installTailwindCSS(languageChoice) {
  ...
  try {
    ...
    const nuxtConfigPath = path.join(process.cwd(), `vite.config.${languageChoice==="ts"?"ts":"js"}`);
