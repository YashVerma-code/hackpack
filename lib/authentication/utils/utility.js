...
export const clerkUserController = (framework, language) => {
  return `${language === 'ts' && (framework === "astro" || framework === "nuxt" ||framework==="next") ? `import type { IUser } from "../database/models/user.model";` : (language==="ts" && framework==="svelte")?`
import UserModel, { type IUser } from "$lib/database/models/User.model";`:``}
${(framework === "astro" || framework === "nuxt" || framework==="next") ? `import UserModel from "../database/models/user.model";
import { connectToDatabase } from "../database/mongoose";`: (framework==="svelte")?`import { connectToDatabase } from "../database/mongoose";`:""}
...
export const POST${language === 'ts' ? ': APIRoute' : ''} = async ({ request }) => {
  try {
    ...
    export const POST ${language==="ts"?`: RequestHandler`:``} = async ({ request }) => {
      ...
      try {
        const evt = webhook.verify(payload, svixHeaders) ${language==="ts"?`as any`:``};
  ...
  return `${language === 'ts' && (framework === "astro") ? `import { verifyWebhook } from '@clerk/astro/webhooks';
import type { APIRoute } from 'astro';
import { createUser, deleteUser, updateUser, type ClerkUserPayload } from '../../../lib/actions/user.action';` : (framework === "astro") ? `
import { verifyWebhook } from '@clerk/astro/webhooks';
import { createUser, deleteUser, updateUser } from '../../../lib/actions/user.action';`: ''}
...
// for clerk vue
export const SIGNIN_COMPONENT = (language) => {
  const isTs = language === "ts";
...
export const SIGNUP_COMPONENT = (language) => {
  const isTs = language === "ts";
...
export const HOME_VIEW_CONTENT = (language, daisy = false) => {
  const isTs = language === "ts";
...
// Clerk Setup for Nuxt.js
// for daisyUI -> badge and btn classes & for shadcn -> (separate components installation)
...
export const webhookCont = () => {
  return `import { verifyWebhook } from '@clerk/nuxt/webhooks'
  import { createUser, deleteUser, updateUser, ClerkUserPayload } from '../actions/user.actions'
...
export const printStepsToConsole = () => {
  console.log(chalk.yellow("Refer the CLERKSETUP.md file for setup instructions"))
}