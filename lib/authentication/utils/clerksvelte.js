...
import { clerkUserController, clerkUserModel, clerkWebhookRoute } from './utility.js';
...
<div class="signup-container bg-gradient-to-br from-blue-500 to-purple-600 w-full">
...
const ROOT_LAYOUT_TS_TWONLY = `<script lang="ts">
  import '../app.css';
  import { Toaster } from 'svelte-sonner';
  import { ClerkProvider } from 'svelte-clerk';
...
`
const ROOT_LAYOUT_JS_TWONLY = `<script>
  import '../app.css';
  import { Toaster } from 'svelte-sonner';
  import { ClerkProvider } from 'svelte-clerk';
...
export async function setupSvelteClerk(state) {
  const { projectName, language ,database,framework} = state;
  ...
  try {
    ...
    // Create root layout
    if (language === 'ts') {
      if (state.uiLibrary === 'twonly') {
        const layoutPath = path.join(routesDir, '+layout.svelte');
        fs.writeFileSync(layoutPath, ROOT_LAYOUT_TS_TWONLY);
      }
      else {
        const layoutPath = path.join(routesDir, '+layout.svelte');
      ...
      if (state.uiLibrary === 'twonly') {
        const layoutPath = path.join(routesDir, '+layout.svelte');
        fs.writeFileSync(layoutPath, ROOT_LAYOUT_JS_TWONLY);
      }
      else {
        const layoutPath = path.join(routesDir, '+layout.svelte');
    ...
    existingContent = existingContent.replace(
      /<div class="w-full min-h-screen bg-gradient-to-b from-\[#1e1b4b\] via-\[#1e3a8a\] to-\[#111827\] flex items-center justify-center p-4">/,
      '<div>'
    );
    ...
    // Add ClerkLoaded + SignedIn wrapper after the outermost div
    existingContent = existingContent.replace(
      /<div[^>]*class="w-full min-h-screen[^"]*"[^>]*>\s*<div[^>]*class="max-w-4xl w-full"[^>]*>/m,
      `<div class="w-full min-h-screen bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center p-4">
  <ClerkLoaded>
    <div class="user-btn absolute top-4 right-4">
      <SignedIn>
        <UserButton afterSignOutUrl="/sign-in" />
      </SignedIn>
    </div>
    ...
    // Properly close the ClerkLoaded section before the final </div>
    existingContent = existingContent.replace(
      /<\/div>\s*<\/div>\s*<\/div>\s*$/m,
      `      </div>
    </div>
    ...
    await execa('npm', ['install', 'svix'], { cwd: rootDir, stdio: 'inherit' });
...
`;
    ...
    if (database === "mongodb") {
      ...
      for (const file of envFiles) {
        ...
        try {
        const webhookDir = path.join(srcDir, "routes", "api", "webhook","clerk");
        fs.mkdirSync(webhookDir, { recursive: true }); 

        const webhookPath = path.join(webhookDir, `+server.${language === "ts" ? "ts" : "js"}`);
        ...
        const signInAllPath = path.join(srcDir,"routes","sign-in","[...catchall]");
        fs.mkdirSync(signInAllPath, { recursive: true });
        const signInAllPathFinal=path.join(signInAllPath,"+page.svelte")
        fs.writeFileSync(signInAllPathFinal, `<script>
...
`);
        ...
        const signUpAllPath = path.join(srcDir,"routes","sign-up","[...catchall]");
        fs.mkdirSync(signUpAllPath, { recursive: true });
        const signUpAllPathFinal=path.join(signUpAllPath,"+page.svelte")
        fs.writeFileSync(signUpAllPathFinal, `<script>
