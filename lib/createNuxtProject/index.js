...
async function createNuxtProject({ projectName: initialProjectName, language, styling, uiLibrary }) {
  ...
  try {
    ...
    if (useTailwind) {
      ...
      const cssDir = path.join(process.cwd(), "app",  "assets", "css");
      const cssFilePath = path.join(cssDir, "tailwind.css");
    ...
    if (uiLibraryChoice !== "none" && uiLibraryChoice !== "tailwind-only") {
      switch (uiLibraryChoice) {
        case "shadcn":
          ...
          break;
        case "daisyui":
          await setupDaisyUI(projectName, language, uiLibraryChoice);
          break
        case "twonly":
          await setupDefault(projectName, useTailwind,language);
          break;
        default:
          console.log(
            chalk.yellow(
