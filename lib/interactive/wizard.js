...
//     return figlet.textSync("Welcome to hackpack !", { font: "Cosmike2" });
//   }
// };
const createResponsiveWelcome = (terminalWidth) => {
  const text =
...
export async function runCli(options = {}) {
  try {
    ...
    while (true) {
      ...
      // Include 'svelte' so users can choose Tailwind vs Plain when creating SvelteKit projects
      if (!state.styling && ['next', 'vite-react', 'svelte','astro','nuxt'].includes(state.framework)) {
      ...
      if (!state.uiLibrary || state.step === 'uiLibrary') {
  ...
  try {
    ...
    const uiLibraries = {
      ...
      astro: {
        ...
        nonTailwind: [{ name: "comming soon", value: 'none' }]
      },
      nuxt:{
        tailwind:[
           { name: 'DaisyUI', value: 'daisyui' },
