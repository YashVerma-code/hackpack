  ...
  try {
    ...
    while (true) {
      ...
      if (!state.framework) {
        ...
        // If user previously chose JavaScript, but selected Nuxt, force TypeScript
        if (state.language === 'js' && state.framework === 'nuxt') {
          state.language = 'ts';
          console.log(chalk.blue('Nuxt requires TypeScript — switching language to TypeScript.'));
        }
      ...
      if (!state.language) {
        ...
        // If user selects JavaScript but framework is Nuxt, override to TypeScript
        if (language === 'js' && state.framework === 'nuxt') {
          state.language = 'ts';
          console.log(chalk.blue('Nuxt requires TypeScript — switching language to TypeScript.'));
        } else {
...
async function askUILibrary(framework, currentStyling) {
  try {
    ...
    if (uiLibraries[framework]) {
      ...
      // If no Tailwind is selected, explain why
