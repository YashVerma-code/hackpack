...
export async function setupMongoDb(projectName, framework, language) {
  console.log(
    chalk.blue("\nSetting up MongoDB backend with Express ...")
  );
  ...
  if (framework === "next") {
    ...
    try {
      ...
      console.log(chalk.cyan("   Mongo db setup : ") + libPath);
      console.log(chalk.cyan("   Environment: ") + ".env.local file created");
    ...
    try {
      ...
      const databasePath = path.join(libPath, "database");
      const modelsPath = path.join(libPath, "models");
      ...
      // Create User model
      fs.writeFileSync(
        path.join(modelsPath, `User.${language === 'ts' ? 'ts' : 'js'}`),
      ...
      console.log(chalk.white("\n📋 Setup Summary:"));
      console.log(chalk.cyan("   Database setup: ") + databasePath);
      console.log(chalk.cyan("   Models: ") + modelsPath);
      console.log(chalk.cyan("   API routes: ") + routesPath);
      console.log(chalk.cyan("   Environment: ") + ".env file created");

      console.log(chalk.green("\n🚀 SvelteKit MongoDB setup completed!"));
      ...
      console.log(chalk.yellow("3. Remember to hash passwords in production!"));
    ...
    try {
      ...
      const databasePath = path.join(serverPath, "db");
      ...
      const userModelContent = userModel;
      fs.writeFileSync(path.join(modelPath, `User.${language === 'ts' ? "ts" : "js"}`), userModelContent)

      const apiCont = apiContent
      fs.writeFileSync(path.join(apiPath, `user.${language === "ts" ? 'ts' : 'js'}`), apiCont);
    ...
    try {
      ...
      if (language === "ts") {
        await execa("npm", ["install", "-D", "typescript", "ts-node", "nodemon", "@types/express", "@types/node", "@types/cors"], { cwd: backendPath, stdio: "inherit" });
