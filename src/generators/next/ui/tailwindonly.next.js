...
export class TailwindOnlyNext extends BaseUILibrary {
  ...
  async setup() {
    ...
    console.log(chalk.blue(`Setting up Tailwind-only project with ${this.language === 'ts' ? 'TypeScript' : 'JavaScript'}...`));
    ...
    try {
      ...
      const buttonExt = this.language === 'ts' ? 'tsx' : 'jsx';
      const buttonComponent = this.language === 'ts'
...
`;
      const buttonComponentPath = path.join(uiComponentDir, `button.${buttonExt}`);
      await fs.writeFile(buttonComponentPath, buttonComponent, 'utf8');

      console.log(chalk.blue('Creating toast component...'));
      const toastComponent = `// Re-export sonner for the Tailwind-only setup
...
`;
      const toastComponentPath = path.join(uiComponentDir, `toaster.${buttonExt}`);
    ...
    <main className="flex min-h-screen flex-col items-center justify-center p-24 bg-gradient-to-b from-slate-900 to-slate-800">
      <div className="z-10 max-w-5xl w-full items-center justify-center text-center">
        ...
        <p className="mt-4 text-sm text-slate-400">
          Edit <code className="font-mono bg-slate-700 p-1 rounded">src/app/${editFile}</code> to get started
        </p>
      </div>
    </main>
    </>
