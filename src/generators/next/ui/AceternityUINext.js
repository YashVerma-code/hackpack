...
export class AceternityUINext extends BaseUILibrary {
  static id = 'aceternityui';
  static displayName = 'Aceternity UI';
  ...
  getDependencies() {
    return ['framer-motion', 'motion', 'clsx', 'tailwind-merge', '@tabler/icons-react', 'sonner'];
  }

  async setup() {
    process.chdir(this.projectName);

    console.log(chalk.blue(`Setting up Aceternity UI with ${this.language === 'ts' ? 'TypeScript' : 'JavaScript'}...`));

    try {
      console.log(chalk.blue('Installing Tailwind CSS and dependencies...'));
      await execa('npm', ['install', '-D', 'tailwindcss@3', 'postcss', 'autoprefixer'], { stdio: 'inherit' });
      ...
      await execa('npm', ['install', 'framer-motion', 'motion', 'clsx', 'tailwind-merge', '@tabler/icons-react'], { stdio: 'inherit' });
...
`, 'utf8');
      ...
      const movingBorderContent = this.language === 'ts'
        ? `"use client";
    ...
    <>
      ...
      if (layoutContent) {
        ...
        if (layoutContent.includes('<html lang="en">')) {
          layoutContent = layoutContent.replace('<html lang="en">', '<html lang="en" className="dark">');
        }
        ...
        <div className="space-y-6">
          ...
          <div className="border-t border-gray-700 mt-6 pt-6">
            <p className="text-gray-400 text-sm mt-4">
              Edit{" "}
              <code className="bg-gray-900 p-1 rounded">src/app/${editFile}</code>{" "}
              to get started
            </p>
          </div>
        </div>
